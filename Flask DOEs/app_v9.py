...
def verificar_diarios_solicitados(periodo_inicio, periodo_fim):
    ...
    while data_atual <= periodo_fim:
        ...
        if os.path.exists(os.path.join('DOEsExtraidos', pdf_solicitado)):
            ...
            pasta_diario_existe = os.path.exists(os.path.join('json extraidos', pasta_data_diario))
        ...
        if(os.path.exists(os.path.join('DOEsExtraidos',P))):
            ...
            for c in range(len(listaconteudos[-1])):
                if '(Continuação)' in listaconteudos[-1][c].nome:
                    # Remove '(Continuação)' e qualquer espaço extra ao final
                    listaconteudos[-1][c].nome = listaconteudos[-1][c].nome.replace('(Continuação)', '').strip()
...
def progresso():
    ...
    return update_progress(total_arquivos, contador_arquivos)
